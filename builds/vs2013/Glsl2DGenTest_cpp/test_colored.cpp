...
static const std::string TITLE = "Colored";
...
std::string testColored();
Test test_colored(TITLE, DESCRIPTION, testColored);

std::string testColored()
{
  // Data initialize
  static const unsigned int DATA_SQUARE_SIZE = DATA_SIZE * DATA_SIZE * 3;
  ...
  if (err != "") { return err; }
  if (!g2dg_setDataType(g2dg, 3)) { return g2dg_getErrorMessageAndDispose(g2dg); }
  ...
  for (int y = 0; y < DATA_SIZE; ++y)
  {
    ...
    {
      sourceData[(x + y * DATA_SIZE) * 3 + 0] = (float)x / DATA_SIZE;
    }
  }
  ...
  print2DFloatArray(sourceData, DATA_SIZE * 3, DATA_SIZE, 3);
  std::cout << "output array: " << std::endl;
  print2DFloatArray(destinationData, DATA_SIZE * 3, DATA_SIZE, 3);
  std::cout << std::endl;
  // Evaluate results
  ...
  for (int y = 0; y < DATA_SIZE; ++y)
  {
    for (int x = 0; x < DATA_SIZE; ++x)
    {
      sourceData[(x + y * DATA_SIZE) * 3 + 0] = 0.0f;
      sourceData[(x + y * DATA_SIZE) * 3 + 1] = (float)x / DATA_SIZE;
    }
  }
  ...
  for (int y = 0; y < DATA_SIZE; ++y)
  {
    for (int x = 0; x < DATA_SIZE; ++x)
    {
      sourceData[(x + y * DATA_SIZE) * 3 + 1] = 0.0f;
      sourceData[(x + y * DATA_SIZE) * 3 + 2] = (float)x / DATA_SIZE;
    }
  }
